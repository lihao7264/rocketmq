...
public class PullAPIWrapper {
    ...
    /**
     * <消息队列,建议下次去哪个brokerId拉取(master/slave)>
     */
    ...
    /**
     * 处理pullResult，进行消息解码、过滤、设置其它属性的操作
     * @param mq            消息队列
    ...
    public PullResult processPullResult(final MessageQueue mq, final PullResult pullResult,
        ...
        if (PullStatus.FOUND == pullResult.getPullStatus()) {
            ...
            /**
             * 3.客户端的消息过滤：如果存在tag 且 不是classFilterMode，则按照tag过滤消息
             */
            if (!subscriptionData.getTagsSet().isEmpty() && !subscriptionData.isClassFilterMode()) {
                msgListFilterAgain = new ArrayList<MessageExt>(msgList.size());
                for (MessageExt msg : msgList) {
                    if (msg.getTags() != null) {
                        // 这采用String#equals方法过滤，而broker端则是比较tagHash值（即hashCode）
            ...
            /**
             * 4.如果有消息过滤钩子，则执行钩子方法，这里可扩展自定义的消息过滤的逻辑
             */
            if (this.hasHook()) {
        ...
        // 6.因为消息已被解析，则设置消息的字节数组为null，释放内存
     ...
     * 拉取消息
     * @param mq               消息队列
     * @param subExpression    订阅关系表达式，仅支持或操作，比如：“tag1 | | tag2 | | tag3”，如果为 null 或 *（表示订阅全部）
     * @param expressionType   订阅关系表达式类型，支持TAG和SQL92，用于过滤
     * @param subVersion       订阅关系版本
     * @param offset           下一个拉取的offset
     * @param maxNums          一次批量拉取的最大消息数（默认值：32）
     * @param sysFlag          系统标记
     * @param commitOffset     提交的消费点位
     * @param brokerSuspendMaxTimeMillis  broker挂起请求的最长时间（默认值：15s）
    ...
    public PullResult pullKernelImpl(
         ...
         * 默认获取master地址
         * 如果有建议的拉取地址，则获取建议的broker地址
         */
        FindBrokerResult findBrokerResult =
            this.mQClientFactory.findBrokerAddressInSubscribe(mq.getBrokerName(),
                this.recalculatePullFromWhichNode(mq), false);
        // 未找到broker地址，则更新topic路由信息再获取一次
        ...
        if (findBrokerResult != null) {
            ...
            // 订阅关系表达式，仅支持或操作，比如：“tag1 | | tag2 | | tag3”，如果为 null 或 *（表示订阅全部）
