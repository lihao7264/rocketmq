...
public class SendMessageProcessor extends AbstractSendMessageProcessor implements NettyRequestProcessor {
    ...
    private CompletableFuture<RemotingCommand> asyncConsumerSendMsgBack(ChannelHandlerContext ctx,
        ...
        /**
         * 2.根据consumerGroup获取对应的重试topic，这里仅获取topic的名字
         *  RocketMQ会为每个消费组都设置一个Topic名称为“%RETRY%+consumerGroup”的重试队列
         *  注意：这里的重试队列是针对消费组，而不是针对每个Topic设置的
         *  每个Consumer实例在启动时，默认订阅了该消费组的重试队列Topic，但只有在真正需用到重试topic时才会创建
         */
        ...
        /**
         * 3.尝试获取或创建重试topic，源码和创建普通topic差不多
         * 区别：重试topic无需模板topic，默认读写队列数都是1，权限为读写
         * 源码参考broker接收消息的源码部分
         */
        ...
        // 如果原消息无该属性，则设置该属性值为正常topic，表示第一次进行重试
        if (null == retryTopic) {
        ...
        /**
         * 6.从订阅关系中获取最大重试次数
         *    如果版本大于3.4.9，则从请求头中获取最大重试次数，这是客户端传递过来的
        ...
        /**
         * 7.如果消息已重试次数 大于等于 最大重试次数 或 延迟等级小于0，则消息不再重试，直接发往死信队列
         */
        if (msgExt.getReconsumeTimes() >= maxReconsumeTimes
             ...
             * 注意：和重试队列一样，这里的死信队列是针对消费组，而不是针对每个Topic设置的。
             */
            newTopic = MixAll.getDLQTopic(requestHeader.getGroup());
            /**
             * 随机选择一个死信队列id
             * 这里是0，因为死信队列数DLQ_NUMS_PER_GROUP是1
             */
            queueIdInt = ThreadLocalRandom.current().nextInt(99999999) % DLQ_NUMS_PER_GROUP;
            /**
             * 7.2 尝试获取 或 创建死信topic，实际上调用的调用重试topic的方法，默认读写队列数都是1，权限为读写
             * 源码参考broker接收消息的源码部分
             */
        ...
        // 设置到PROPERTY_ORIGIN_MESSAGE_ID属性中
        MessageAccessor.setOriginMessageId(msgInner, UtilAll.isBlank(originMsgId) ? msgExt.getMsgId() : originMsgId);
        msgInner.setPropertiesString(MessageDecoder.messageProperties2String(msgExt.getProperties()));
        /**
         * 9.异步方式将消息存储到commitLog中
         *   在该方法中，将会处理延迟消息的逻辑。
         *   如果是延迟消息（即DelayTimeLevel大于0）
         *   则替换topic为SCHEDULE_TOPIC_XXXX，替换queueId为延迟队列id（id = level - 1），保存真实topic和queueId，方便后面恢复。
         */
    ...
    private boolean handleRetryAndDLQ(SendMessageRequestHeader requestHeader, RemotingCommand response,
        ...
        // 如果是重试topic，则顺序消费重试超过最大次数时发送的消息的topic
